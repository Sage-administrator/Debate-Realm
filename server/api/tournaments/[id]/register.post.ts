...
import { getUserFromEvent, getUserFromEventWithSession, type JWTPayload } from '../../../utils/auth'
...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (!tournament.registrationOpen) {
      throw createError({ statusCode: 400, statusMessage: '该赛事未开放报名' })
    }
    ...
    if (!user && !tournament.isPublic) {
      throw createError({ statusCode: 401, statusMessage: '未登录用户无权报名该赛事，请先登录' })
    }

    // 8. 校验：队伍报名必须提供队伍名称
    if (type === 'team' && !teamName?.trim()) {
      throw createError({ statusCode: 400, statusMessage: '队伍报名必须填写队伍名称' })
    }

    // 9. 校验：至少需要 1 名报名成员
    if (!members || members.length === 0) {
      throw createError({ statusCode: 400, statusMessage: '至少需要 1 名报名成员' })
    }
    ...
    if (type === 'team' && tournament.teamSize && members.length > tournament.teamSize) {
      throw createError({
        statusCode: 400,
        statusMessage: `队伍成员数不能超过 ${tournament.teamSize} 人`,
      })
    }

    // 11. 校验：提交人姓名与联系电话必填
    if (!submitterName?.trim()) {
      throw createError({ statusCode: 400, statusMessage: '提交人姓名不能为空' })
    }
    if (!contactPhone?.trim()) {
      throw createError({ statusCode: 400, statusMessage: '联系电话不能为空' })
    }
    ...
    if (user) {
      ...
      if (existing) {
        throw createError({ statusCode: 409, statusMessage: '您已报名该赛事，请勿重复报名' })
      }
    }
    ...
    throw createError({ statusCode: 500, statusMessage: '提交报名失败' })
  }
})
