...
function mapQuestionsToFields(questions: any[]): any[] {
  return (questions || []).map((q) => ({
    ...
    fieldOptions: typeof q.options === 'string' ? q.options : (q.options ? JSON.stringify(q.options) : null),
    required: q.required,
...
async function loadDesignIntoFields() {
  ...
  if (scoreTemplate.value?.id) {
    try {
      const res = await fetch(`/api/tournaments/${tournamentId.value}/questionnaires/${scoreTemplate.value.id}`, {
      ...
      designerFormSettings.thankYouText = settings.thankYouText || '感谢您的评分！您可重新提交以更新结果。'
    ...
    designerFields.value = [{
      id: '', fieldName: '请为本次比赛打分', fieldKey: `field_scale_${Date.now().toString(36)}`,
      fieldType: 'scale', fieldOptions: null, required: true, sortOrder: 0,
      appliesTo: 'both', placeholder: '', description: '', width: 'full', systemField: false,
...
watch(activeTab, (t) => { if (t === 'design') loadDesignIntoFields() })
  ...
  <div>
    ...
    <div v-if="!loading && (!canDesign || activeTab === 'stats')">
                ...
                >
                  <td class="py-3 px-4">
                    <div class="flex items-center gap-2">
                      ...
                      <span v-else class="text-sm font-medium text-[var(--color-text-muted)]">{{ Number(index) + 1 }}</span>
                    </div>
                  </td>
                  <td class="py-3 px-4">
              ...
              <div class="space-y-2">
                ...
                >
                  <div class="flex items-center gap-3">
                    <span class="text-xs font-medium text-[var(--color-text-muted)] w-6">{{ Number(index) + 1 }}</span>
        ...
        <UCard class="mb-6" v-if="stats?.bestDebaters?.length">
          ...
          <div class="space-y-3">
            <div
              ...
              :class="Number(index) < 3 ? 'bg-[var(--color-accent-bg)]' : 'bg-[var(--color-bg-secondary)]'"
            >
              <div class="flex items-center gap-3">
                <div :class="[
        ...
        <UCard class="mb-6" v-if="stats?.judgeRankings?.length">
          ...
          <div class="space-y-3">
            ...
            >
              <span class="text-sm font-medium text-[var(--color-text-muted)] w-6">{{ Number(index) + 1 }}</span>
              <div class="flex-1">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                >
                  ...
                  <div class="mt-3">
                    ...
                    <!-- 去评分 -->
                    <div v-if="match.status === 'finished'" class="mt-3 pt-2 border-t border-[var(--color-border)]">
                      <UButton
                        size="xs"
                        color="primary"
                        variant="soft"
                        @click="void navigateTo(`/tournaments/${tournamentId}/score-survey/${match.id}`)"
                      >
                        <UIcon name="i-lucide-star" class="w-3.5 h-3.5 mr-1" />
                        去评分{{ submissionCountByMatch[match.id] ? ` (${submissionCountByMatch[match.id]})` : '' }}
                      </UButton>
                    </div>
                  </div>
                </div>
        ...
        <!-- ═══ 评分统计 ═══ -->
        <UCard
          v-if="scoreStats?.hasTemplate && scoreStats.matches?.some((m: any) => m.submissionCount > 0)"
          class="mb-6"
        >
          <template #header>
            <div class="flex items-center justify-between">
              <h2 class="text-base font-semibold text-[var(--color-text-primary)] flex items-center gap-2">
                <UIcon name="i-lucide-bar-chart-3" class="w-4 h-4 text-[var(--color-accent-primary)]" />
                评分统计
              </h2>
            ...
            >
              <div class="flex items-center justify-between mb-2">
                <div class="flex items-center gap-2 text-sm">
                  <span class="font-medium text-[var(--color-text-primary)]">{{ matchOf(m.matchId)?.teamA || '待定' }}</span>
                  <span class="text-[var(--color-text-muted)] text-xs">VS</span>
                  <span class="font-medium text-[var(--color-text-primary)]">{{ matchOf(m.matchId)?.teamB || '待定' }}</span>
                </div>
                <span class="text-xs text-[var(--color-text-muted)]">{{ m.submissionCount }} 人评分</span>
              </div>
              ...
              <div class="space-y-3 pl-1">
                <div v-for="s in m.scales" :key="s.fieldKey">
                  ...
                  <div class="flex items-end gap-1 h-12">
                    ...
                    >
                      <span class="text-[10px] text-[var(--color-text-muted)] mb-0.5">{{ d.count }}</span>
                      <div
                      ...
                      ></div>
                      <span class="text-[10px] text-[var(--color-text-muted)] mt-0.5">{{ d.value }}</span>
                    </div>
                  </div>
                </div>
              </div>
    ...
    <div v-if="!loading && canDesign && activeTab === 'design'" class="space-y-6">
      ...
      <UCard v-if="scoreTemplate" class="mb-6">
        <template #header>
          <h2 class="text-base font-semibold text-[var(--color-text-primary)] flex items-center gap-2">
            <UIcon name="i-lucide-clipboard-list" class="w-4 h-4 text-[var(--color-accent-primary)]" />
            评分问卷设计
          </h2>
        </template>
        <div class="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
          <span class="text-[var(--color-text-secondary)]">题目数：<b class="text-[var(--color-text-primary)]">{{ scoreTemplate.questionCount }}</b></span>
          <span class="text-[var(--color-text-secondary)]">提交数：<b class="text-[var(--color-text-primary)]">{{ scoreTemplate.submissionCount }}</b></span>
          <span class="text-[var(--color-text-secondary)]">受众：<b class="text-[var(--color-text-primary)]">{{ audienceLabels[scoreTemplate.settings?.audience as keyof typeof audienceLabels] || '登录用户' }}</b></span>
          <UBadge :color="scoreTemplate.status === 'open' ? 'success' : 'neutral'" size="xs" variant="soft">{{ scoreTemplate.status === 'open' ? '收集中' : scoreTemplate.status }}</UBadge>
        </div>
      </UCard>
      <p v-else class="text-sm text-[var(--color-text-muted)] mb-4">尚未设计评分问卷。添加量表题并选择可填写人群，即可在对每场已结束比赛下收集评分。</p>
      ...
      <div class="flex flex-wrap items-center gap-x-6 gap-y-3 mb-2">
        <div>
          ...
          <ClientOnly>
            ...
            <template #fallback><div class="w-full h-8 rounded-lg bg-[var(--color-bg-secondary)] border border-[var(--color-border)]"></div></template>
          </ClientOnly>
        </div>
        <label class="flex items-center gap-2 text-xs text-[var(--color-text-secondary)] cursor-pointer pt-5">
          <input type="checkbox" v-model="designerAllowMultiple" class="rounded border-[var(--color-border)]" />
          允许同一人多次提交（重复提交将覆盖上次）
        </label>
      </div>
      ...
      <div class="border border-[var(--color-border)] rounded-lg overflow-hidden h-[720px]">
        <ClientOnly>
          ...
          />
          <template #fallback><div class="h-full flex items-center justify-center text-[var(--color-text-muted)] text-sm">加载设计器…</div></template>
        </ClientOnly>
      </div>
