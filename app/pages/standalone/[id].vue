...
const {
  get: getStandaloneMatch,
  createMatch: createStandaloneMatchMatch,
  delete: deleteStandaloneMatch,
} = useStandaloneMatches()
...
async function load() {
  loading.value = true
  try {
    match.value = await getStandaloneMatch(id.value)
  } catch (e: any) {
...
async function handleDelete() {
  ...
  try {
    await deleteStandaloneMatch(id.value)
    toast.add({ title: '已删除', color: 'success' })
    await navigateTo('/standalone')
  } catch (e: any) {
  ...
  <div class="min-h-screen">
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <template v-else-if="match">
        <div class="flex items-start justify-between mb-6">
          <div>
            ...
            <p v-if="match.description" class="text-sm text-white/50 mt-1">
              {{ match.description }}
            </p>
          </div>
        ...
        <div class="flex items-center justify-between mb-4">
          ...
          <UButton color="primary" size="sm" @click="void (showCreateMatch = true)"
            >添加场次</UButton
          >
        </div>
        ...
        <div class="glass-card p-6">
          ...
          >
            <template #match-data="{ row }"
              >{{ r(row).teamA || '正方' }} vs {{ r(row).teamB || '反方' }}</template
            >
            <template #score-data="{ row }">
              <span v-if="r(row).status === 'finished'"
                >{{ r(row).scoreA }} : {{ r(row).scoreB }}</span
              >
            ...
            <template #actions-data="{ row }">
              <div class="flex gap-1">
                <UButton
                  v-if="r(row).status !== 'finished'"
                  color="success"
                  variant="ghost"
                  size="xs"
                  @click="openResult(r(row).id)"
                  >登记赛果</UButton
                >
                <UButton color="neutral" variant="ghost" size="xs" :to="`/timer/${r(row).id}`"
                  >计时器</UButton
                >
              </div>
            </template>
        ...
        <template #body>
          <div class="space-y-4">
            <UFormField label="轮次" required
              ><UInput v-model="matchForm.round" placeholder="如：立论陈词"
            /></UFormField>
            <UFormField label="顺序" required
              ><UInput v-model.number="matchForm.orderNum" type="number" placeholder="1"
            /></UFormField>
            <UFormField label="正方"
              ><UInput v-model="matchForm.teamA" placeholder="正方"
            /></UFormField>
            <UFormField label="反方"
              ><UInput v-model="matchForm.teamB" placeholder="反方"
            /></UFormField>
          </div>
        </template>
        <template #footer>
          <div class="flex justify-end gap-2">
            <UButton color="neutral" variant="outline" @click="void (showCreateMatch = false)"
              >取消</UButton
            >
      ...
      <UModal v-model:open="showResult" title="登记赛果">
        ...
        <template #footer>
          <div class="flex justify-end gap-2">
            <UButton color="neutral" variant="outline" @click="void (showResult = false)"
              >取消</UButton
            >
