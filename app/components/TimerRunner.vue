...
<template>
  ...
  >
    ...
    <SpeechPermissionPanel v-if="hostType === 'tournament' && isQQBotTeam" />
  </div>
</template>
...
const showSetup = ref(props.showSetupPanel ?? true)
...
async function openMatchSelectModal() {
  ...
  if (matches.value.length === 0) {
    ...
    try {
      const res = await $fetch<any>(`/api/tournaments/${hostId.value}/matches`, {
...
async function loadData() {
  try {
    // 加载宿主基本信息（赛事 / 单场），用于标题兜底与队伍名预填
    let hostName = '辩论赛'
    if (props.hostType === 'tournament') {
      const tournament = await $fetch<any>(`/api/tournaments/${hostId.value}`, {
        headers: { Authorization: `Bearer ${useAuthStore().token}` },
      })
      if (tournament?.data) {
      ...
      const match = await $fetch<any>(`/api/standalone-matches/${hostId.value}`, {
        headers: { Authorization: `Bearer ${useAuthStore().token}` },
      })
      if (match?.data) {
        const m = match.data
    ...
    await loadSharedConfig(hostId.value, props.hostType)
...
onMounted(async () => {
  await loadData()
  // 预览模式下自动应用已加载的队伍/辩题设置，跳过设置弹窗（仅正式赛事页有设置面板）
  if (props.hostType === 'tournament' && isPreviewMode.value) {
    positiveTopic.value = setupPositiveTopic.value
    negativeTopic.value = setupNegativeTopic.value
    teamPositiveName.value = setupTeamPositiveName.value
    teamNegativeName.value = setupTeamNegativeName.value
