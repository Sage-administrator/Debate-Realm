...
export function useRegistration() {
  ...
  return {
    getRegistrationConfig: (tournamentId: string) =>
      api.tournaments.registration.config(tournamentId),
    submitRegistration: (tournamentId: string, data: any) =>
      api.tournaments.registration.submit(tournamentId, data),
    ...
    getRegistrations: (tournamentId: string, params?: { status?: string; type?: string }) =>
      api.tournaments.registration
        .list(tournamentId, params)
        .then((res) => (res as any).registrations || []),
    reviewRegistration: (
      tournamentId: string,
      regId: string,
