...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (username.length < 2) {
      throw createError({ statusCode: 400, statusMessage: '用户名至少2个字符' })
    }
    ...
    if (mode === 'individual' && role !== 'individual') {
      throw createError({ statusCode: 400, statusMessage: '个人模式只能选择个人用户角色' })
    }

    if (mode === 'system' && role !== 'system_admin') {
      throw createError({ statusCode: 400, statusMessage: '系统管理模式只能选择系统管理员角色' })
    }

    // 团队模式（team/qq_bot）下，角色必须为 admin 或 subaccount
    if ((mode === 'team' || mode === 'qq_bot') && !['admin', 'subaccount'].includes(role)) {
      throw createError({ statusCode: 400, statusMessage: '团队模式下角色必须为团队管理员或子账号' })
    }
    ...
    if ((role === 'admin' || role === 'subaccount') && teamId) {
      ...
      if (!team) {
        throw createError({ statusCode: 400, statusMessage: '团队不存在' })
      }
      // 子账号必须有 teamId
    } else if (role === 'subaccount' && !teamId) {
      throw createError({ statusCode: 400, statusMessage: '子账号必须选择所属团队' })
    }

    // 系统管理员角色不应有 teamId
    if (role === 'system_admin' && teamId) {
      throw createError({ statusCode: 400, statusMessage: '系统管理员不应关联团队' })
    }

    // 个人用户不应有 teamId
    if (role === 'individual' && teamId) {
      throw createError({ statusCode: 400, statusMessage: '个人用户不应关联团队' })
    }
