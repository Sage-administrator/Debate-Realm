...
function roleColor(role: string): 'primary' | 'neutral' | 'info' | 'error' {
  const m: Record<string, 'primary' | 'neutral' | 'info' | 'error'> = {
    admin: 'primary',
    subaccount: 'info',
    system_admin: 'error',
    individual: 'neutral',
...
async function handleCleanupMembers() {
  ...
  if (
    !confirm(
      `⚠️ 确定要删除该团队的所有子账号吗？（共 ${members.value.filter((m: any) => m.role === 'subaccount').length} 个）\n\n此操作将删除所有子账号用户及其数据，不可撤销！`,
    )
  )
    return
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 fade-in">
      <div v-if="loading" class="text-center py-12">
        <UIcon
          ...
          class="w-8 h-8 animate-spin mx-auto text-indigo-600 dark:text-indigo-400"
        />
      </div>
      ...
      <template v-else-if="team">
        ...
        <div class="flex items-start justify-between mb-8">
          <div>
            ...
            <div class="flex items-center gap-2 mt-2">
              <UBadge
                :label="team.mode === 'qq_bot' ? 'QQ频道' : '普通团队'"
                :color="team.mode === 'qq_bot' ? 'primary' : 'neutral'"
                size="sm"
                variant="soft"
              />
              <span class="text-sm text-[var(--color-text-muted)]"
                >{{ team.tournaments?.length ?? 0 }} 个赛事 ·
                {{ team.memberCount ?? 0 }} 个成员</span
              >
              <!-- 成员数量过多警告 -->
              <UBadge
                v-if="(team.memberCount ?? 0) > 100"
                color="warning"
                size="xs"
                variant="solid"
              >
            ...
            <div v-else class="divide-y divide-white/10">
              ...
              >
                <div class="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div
                    class="w-9 h-9 rounded-full bg-indigo-500/20 flex items-center justify-center flex-shrink-0 overflow-hidden border border-[var(--color-border)]"
                  >
                    <img
                      v-if="m.avatar"
                      :src="m.avatar"
                  ...
                  <div class="min-w-0 flex-1">
                    <div class="flex items-center gap-2">
                      ...
                      <!-- 昵称与用户名不同时显示用户名（@xxx） -->
                      <span
                        v-if="m.nickname && m.nickname !== m.username"
                    ...
                    <div
                      class="text-xs text-[var(--color-text-muted)] flex items-center gap-1 flex-wrap"
                    >
                      <UBadge
                        :label="roleCN(m.role)"
              ...
              <!-- 成员过多时显示展开按钮 -->
              <div
                v-if="members.length > MEMBER_DISPLAY_LIMIT && !showAllMembers"
        ...
        <UModal
          v-model:open="showEditMember"
          :title="editingMember ? `编辑成员 - ${memberDisplayName(editingMember)}` : '编辑成员'"
        >
          <template #body>
            <div class="space-y-4">
              <!-- 头像预览 -->
              <div class="flex items-center gap-3">
                <div
                  class="w-14 h-14 rounded-full bg-indigo-500/20 flex items-center justify-center flex-shrink-0 overflow-hidden border border-[var(--color-border)]"
                >
                  <img
          ...
          <template #footer>
            <div class="flex justify-end gap-2">
              <UButton
                ...
                >取消</UButton
              >
              <UButton color="primary" :loading="savingMember" @click="handleUpdateMember"
                >保存</UButton
              >
            </div>
          </template>
        ...
        <UModal
          ...
          :title="resettingMember ? `重置密码 - ${memberDisplayName(resettingMember)}` : '重置密码'"
        >
          <template #body>
            <div class="space-y-4">
          ...
          <template #footer>
            <div class="flex justify-end gap-2">
              <UButton
                ...
                >取消</UButton
              >
              <UButton color="primary" :loading="resettingPassword" @click="handleResetPassword"
                >确认重置</UButton
              >
            </div>
          </template>
