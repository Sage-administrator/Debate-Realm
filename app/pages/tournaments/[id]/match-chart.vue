  ...
  <div>
    ...
    <div v-else>
        ...
        <UCard class="mb-6" v-if="matchesByRound.length">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="text-center p-4 rounded-xl bg-[var(--color-bg-secondary)]">
              ...
              <p class="text-2xl font-bold text-[var(--color-text-primary)]">{{ matchesByRound.length }}</p>
            </div>
            <div class="text-center p-4 rounded-xl bg-[var(--color-bg-secondary)]">
              <p class="text-xs text-[var(--color-text-muted)] mb-1">总场次</p>
              <p class="text-2xl font-bold text-[var(--color-text-primary)]">{{ statusStats.total }}</p>
            </div>
            <div class="text-center p-4 rounded-xl bg-[var(--color-bg-secondary)]">
              <p class="text-xs text-[var(--color-text-muted)] mb-1">已完成</p>
              <p class="text-2xl font-bold text-[var(--color-accent-success)]">{{ statusStats.finished }}</p>
            </div>
            <div class="text-center p-4 rounded-xl bg-[var(--color-bg-secondary)]">
              <p class="text-xs text-[var(--color-text-muted)] mb-1">进行中</p>
              <p class="text-2xl font-bold text-[var(--color-accent-warning)]">{{ statusStats.ongoing }}</p>
            </div>
          </div>
        </UCard>
        ...
        <div v-if="matchesByRound.length" class="space-y-8">
          <div
            v-for="(roundData, roundIndex) in matchesByRound"
            :key="roundData.round"
          >
            <!-- 轮次标题 -->
            <div class="flex items-center gap-3 mb-4">
              <span class="text-xs px-3 py-1 rounded-full bg-[var(--color-accent-bg)] text-[var(--color-accent-primary)] font-medium">
                第 {{ roundData.round }} 轮
              </span>
              <span class="text-sm font-semibold text-[var(--color-text-primary)]">{{ getRoundName(roundData.round) }}</span>
                ...
                <div class="mt-2">
                  ...
                  <div class="flex items-center justify-between mb-3">
                    <div class="flex items-center gap-2">
                      <div class="w-8 h-8 rounded-full bg-[var(--color-accent-bg)] flex items-center justify-center">
                  ...
                  <div class="flex items-center justify-between mt-3">
                    <div class="flex items-center gap-2">
                      <div class="w-8 h-8 rounded-full bg-[var(--color-error-bg)] flex items-center justify-center">
                  ...
                  <div v-if="match.topic" class="mt-3 pt-3 border-t border-[var(--color-border)]">
                    ...
                    <p class="text-sm text-[var(--color-text-secondary)] line-clamp-2">{{ match.topic }}</p>
                  </div>
                  ...
                  <!-- 最佳辩手 -->
                  <div v-if="match.status === 'finished' && (match.bestDebaterA || match.bestDebaterB)" class="mt-3 pt-3 border-t border-[var(--color-border)]">
                    <div class="flex flex-wrap gap-2">
                      <span v-if="match.bestDebaterA" class="text-xs px-2 py-0.5 rounded bg-[var(--color-accent-bg)] text-[var(--color-accent-primary)]">
                        最佳辩手(正): {{ match.bestDebaterA }}
                      </span>
                      <span v-if="match.bestDebaterB" class="text-xs px-2 py-0.5 rounded bg-[var(--color-error-bg)] text-[var(--color-accent-error)]">
        ...
        <UCard v-if="matchesByRound.length > 1" class="mt-8">
          ...
          <div class="flex flex-wrap items-center justify-center gap-4 py-6">
            ...
            >
              ...
              <div v-if="idx < matchesByRound.length - 1" class="w-8 h-8 flex items-center justify-center">
        ...
        <div v-if="!loading && matches.length === 0" class="text-center py-12">
          <UIcon name="i-lucide-swords" class="w-16 h-16 text-[var(--color-text-muted)] mx-auto mb-4" />
