...
import type {
  ...
  TimerProjectInfo,
  TimerTemplate,
  CreateTimerProjectRequest,
  UpdateTimerProjectRequest,
...
import type {
  ...
  MyVoteRecordResult,
  VoteStatsResult,
} from '#shared/schemas/topic-votes'
...
export function useApi() {
  ...
  const auth_api = {
    ...
    confirmLogin: (body: ConfirmLoginRequest) =>
      post<LoginResponse>('/api/auth/confirm-login', null, body),
  ...
  const teams = {
    ...
    members: {
      ...
      create: (teamId: string, body: CreateMemberRequest) =>
        post(`/api/teams/${teamId}/members`, t(), body),
      delete: (teamId: string, userId: string) =>
        del(`/api/teams/${teamId}/members/${userId}`, t()),
      ...
      resetPassword: (teamId: string, userId: string, body: ResetMemberPasswordRequest) =>
        put<{ message: string }>(
          `/api/teams/${teamId}/members/${userId}/reset-password`,
          t(),
          body,
        ),
      cleanup: (teamId: string) =>
        del<{ deleted: number; message: string }>(`/api/teams/${teamId}/members/cleanup`, t()),
    },
    tournaments: {
  ...
  const tournaments = {
    ...
    publicList: (query: PublicTournamentListQuery) =>
      get<PublicTournamentInfo[]>(
    ...
    registration: {
      config: (tournamentId: string) =>
        get<RegistrationConfig>(`/api/tournaments/${tournamentId}/registration-config`, t()),
      settings: (tournamentId: string, body: RegistrationSettings) =>
        put(`/api/tournaments/${tournamentId}/registration-settings`, t(), body),
      fields: (tournamentId: string, fields: Record<string, unknown>[]) =>
        put<SaveRegistrationFieldsResult>(
          `/api/tournaments/${tournamentId}/registration-fields`,
          t(),
          { fields },
        ),
      ...
      my: (tournamentId: string) =>
        get<RegistrationRecord[]>(`/api/tournaments/${tournamentId}/my-registration`, t()),
      submit: (tournamentId: string, body: SubmitRegistrationRequest) =>
        post<{ code: number; message: string; data: RegistrationRecord }>(
      ...
      createDebaterAccounts: (tournamentId: string, registrationIds?: string[]) =>
        post(`/api/tournaments/${tournamentId}/registrations/create-accounts`, t(), {
          registrationIds,
        }),
    },
    ...
    matches: {
      list: (tournamentId: string) =>
        get<MatchInfo[]>(`/api/tournaments/${tournamentId}/matches`, t()),
      create: (tournamentId: string, body: CreateMatchRequest) =>
        post(`/api/tournaments/${tournamentId}/matches`, t(), body),
      generate: (tournamentId: string, body: GenerateMatchesRequest) =>
        post(`/api/tournaments/${tournamentId}/matches/generate`, t(), body),
      drawLots: (tournamentId: string, body: DrawLotsRequest) =>
        post<DrawLotsResult>(`/api/tournaments/${tournamentId}/draw-lots`, t(), body),
    },
    ...
    scores: {
      get: (id: string, params?: { matchId?: string; type?: string }) =>
        get<Record<string, unknown>[]>(
          `/api/tournaments/${id}/scores`,
          t(),
          params as Record<string, unknown>,
        ),
      submit: (id: string, body: SubmitScoreRequest) =>
        post(`/api/tournaments/${id}/scores`, t(), body),
    },

    // Timer
    timer: {
      config: (id: string) => get<TimerConfig>(`/api/tournaments/${id}/timer-config`, t()),
      saveConfig: (id: string, body: TimerConfig) =>
        put(`/api/tournaments/${id}/timer-config`, t(), body),
      template: (id: string) => get<TimerTemplate>(`/api/tournaments/${id}/timer-template`, t()),
      saveTemplate: (id: string, body: TimerTemplate) =>
        put(`/api/tournaments/${id}/timer-template`, t(), body),
    },

    // Topics (debate)
    topics: {
      list: (id: string, params?: { search?: string; category?: string }) =>
        get<DebateTopicListResult>(
          `/api/tournaments/${id}/debate-topics`,
          t(),
          params as Record<string, unknown>,
        ),
      create: (id: string, body: Record<string, unknown>) =>
        post(`/api/tournaments/${id}/debate-topics`, t(), body),
      update: (id: string, topicId: string, body: Record<string, unknown>) =>
        put(`/api/tournaments/${id}/debate-topics/${topicId}`, t(), body),
      delete: (id: string, topicId: string) =>
        del(`/api/tournaments/${id}/debate-topics/${topicId}`, t()),
    ...
    votes: {
      list: (id: string, params?: { status?: string; matchId?: string }) =>
        get<TopicVoteListResult>(
          `/api/tournaments/${id}/topic-votes`,
          t(),
          params as Record<string, unknown>,
        ),
      get: (id: string, voteId: string) =>
        get<TopicVoteDetail>(`/api/tournaments/${id}/topic-votes/${voteId}`, t()),
      create: (id: string, body: Record<string, unknown>) =>
        post(`/api/tournaments/${id}/topic-votes`, t(), body),
      update: (id: string, voteId: string, body: Record<string, unknown>) =>
        put(`/api/tournaments/${id}/topic-votes/${voteId}`, t(), body),
      delete: (id: string, voteId: string) =>
        del(`/api/tournaments/${id}/topic-votes/${voteId}`, t()),
      cast: (id: string, voteId: string, body: Record<string, unknown>) =>
        post(`/api/tournaments/${id}/topic-votes/${voteId}/cast`, t(), body),
      myRecord: (id: string, voteId: string) =>
        get<MyVoteRecordResult>(`/api/tournaments/${id}/topic-votes/${voteId}/my-record`, t()),
      stats: (id: string, voteId: string) =>
        get<VoteStatsResult>(`/api/tournaments/${id}/topic-votes/${voteId}/stats`, t()),
    },
  ...
  const matches = {
    ...
    restore: (id: string, currentVersion: number) =>
      post(`/api/matches/${id}/restore`, t(), { currentVersion }),
    submitResult: (id: string, body: SubmitResultRequest) =>
      post<MatchResultSubmitResult>(`/api/matches/${id}/result`, t(), body),
  ...
  const standalone = {
    ...
    update: (id: string, body: Record<string, unknown>) =>
      put(`/api/standalone-matches/${id}`, t(), body),
    delete: (id: string) => del(`/api/standalone-matches/${id}`, t()),
    createMatch: (id: string, body: CreateMatchRequest) =>
      post(`/api/standalone-matches/${id}/matches`, t(), body),
    timer: {
      config: (id: string) => get<TimerConfig>(`/api/standalone-matches/${id}/timer-config`, t()),
      saveConfig: (id: string, body: TimerConfig) =>
        put(`/api/standalone-matches/${id}/timer-config`, t(), body),
      template: (id: string) =>
        get<TimerTemplate>(`/api/standalone-matches/${id}/timer-template`, t()),
      saveTemplate: (id: string, body: TimerTemplate) =>
        put(`/api/standalone-matches/${id}/timer-template`, t(), body),
    },
  }
  ...
  const timerProjects = {
    ...
    update: (id: string, body: UpdateTimerProjectRequest) =>
      put(`/api/timer/projects/${id}`, t(), body),
  ...
  const bot = {
    ...
    subchannels: (guildId: string) =>
      get<ChannelInfo[]>(`/api/bot/channels/${guildId}/subchannels`, t()),
    ...
    arena: {
      ...
      status: (channelId?: string) =>
        get('/api/bot/arena/status', t(), channelId ? { channelId } : undefined),
      close: (channelId: string, guildId?: string) =>
        post('/api/bot/arena/close', t(), { channelId, guildId }),
    ...
    permissions: {
      ...
      revokeSpeak: (body: RevokeSpeakRequest) =>
        post('/api/bot/permissions/revoke-speak', t(), body),
      reset: (channelId: string) => post('/api/bot/permissions/reset', t(), { channelId }),
      switchRound: (targetSide: string, channelId: string) =>
        post('/api/bot/permissions/switch-round', t(), { targetSide, channelId }),
    },
  }
  ...
  const scheduledPosts = {
    ...
    update: (id: string, body: Record<string, unknown>) =>
      put(`/api/scheduled-posts/${id}`, t(), body),
    delete: (id: string) => del(`/api/scheduled-posts/${id}`, t()),
    toggle: (id: string, paused: boolean) =>
      post(`/api/scheduled-posts/${id}/toggle`, t(), { paused }),
