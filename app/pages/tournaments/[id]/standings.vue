  ...
  <div class="min-h-screen bg-[var(--color-bg-primary)]">
    ...
    <main class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <template v-else-if="data">
          ...
          <div class="p-8">
            <div class="flex flex-wrap items-start justify-between gap-4 mb-6">
              <div>
                ...
                <h1 class="text-3xl font-bold text-[var(--color-text-primary)]">{{ data.tournament.name }}</h1>
                <p class="text-[var(--color-text-muted)] mt-2">主办方：{{ data.tournament.organizer }}</p>
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div class="bg-[var(--color-bg-secondary)] rounded-xl p-4 text-center">
                <div class="text-2xl font-bold text-[var(--color-text-primary)]">{{ data.matchStats.total }}</div>
        ...
        <!-- ═══ 总积分榜 ═══ -->
        <div v-if="activeTab === 'overall'" class="bg-[var(--color-bg-secondary)] backdrop-blur rounded-2xl border border-[var(--color-border)] overflow-hidden">
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="px-6 py-4">
                    <div class="flex items-center gap-3">
                      <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-500 to-cyan-400 flex items-center justify-center text-white text-xs font-bold">
                        {{ team.name.charAt(0) }}
                      </div>
                      <div>
                        <div class="text-[var(--color-text-primary)] font-medium">{{ team.name }}</div>
                ...
                <tr v-if="data.standings.length === 0">
                  <td colspan="10" class="px-6 py-12 text-center text-[var(--color-text-muted)]">
                    <UIcon name="i-lucide-trophy" class="w-12 h-12 mx-auto mb-3 text-[var(--color-border-muted)]" />
                    暂无积分数据
                  </td>
                </tr>
              </tbody>
        ...
        <div v-if="activeTab === 'groups'">
          <div v-if="data.groups.length === 0" class="bg-[var(--color-bg-secondary)] backdrop-blur rounded-2xl border border-[var(--color-border)] p-12 text-center">
            <UIcon name="i-lucide-layout-grid" class="w-12 h-12 mx-auto mb-3 text-[var(--color-border-muted)]" />
            ...
            >
              <div class="px-6 py-4 border-b border-[var(--color-border)] flex items-center gap-3">
                <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-500 to-cyan-400 flex items-center justify-center">
                  <span class="text-white text-xs font-bold">{{ group.label.replace('组', '') }}</span>
                </div>
                <h3 class="text-lg font-semibold text-[var(--color-text-primary)]">{{ group.label }}</h3>
                <span class="text-[var(--color-text-muted)] text-sm ml-auto">{{ group.teams.length }} 支队伍</span>
              </div>
              <div class="overflow-x-auto">
                <table class="w-full">
                  <thead>
                    <tr class="border-b border-[var(--color-border-muted)] text-[var(--color-text-muted)] text-xs">
          ...
          <div class="bg-[var(--color-bg-secondary)] backdrop-blur rounded-2xl border border-[var(--color-border)] overflow-hidden">
            ...
            <div class="p-6">
              <div v-if="data.bestDebaters.length === 0" class="text-center py-8">
                <UIcon name="i-lucide-star" class="w-12 h-12 mx-auto mb-3 text-[var(--color-border-muted)]" />
              ...
              <div v-else class="space-y-3">
                ...
                >
                  <div :class="['w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold', idx < 3 ? 'bg-gradient-to-br from-amber-500 to-orange-400 text-white' : 'bg-[var(--color-bg-tertiary)] text-[var(--color-text-secondary)]']">
                    {{ idx + 1 }}
                  </div>
                  <div class="flex-1">
          ...
          <div class="bg-[var(--color-bg-secondary)] backdrop-blur rounded-2xl border border-[var(--color-border)] overflow-hidden">
            ...
            <div class="p-6">
              <div v-if="data.judgeRankings.length === 0" class="text-center py-8">
                <UIcon name="i-lucide-gavel" class="w-12 h-12 mx-auto mb-3 text-[var(--color-border-muted)]" />
              ...
              <div v-else class="space-y-3">
                <div
                  ...
                  class="flex items-center gap-4"
                >
                  <div :class="['w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold', idx < 3 ? 'bg-gradient-to-br from-blue-500 to-cyan-400 text-white' : 'bg-[var(--color-bg-tertiary)] text-[var(--color-text-secondary)]']">
                    {{ idx + 1 }}
                  </div>
                  <div class="flex-1">
