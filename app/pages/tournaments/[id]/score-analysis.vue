...
function exportDimensionCSV() {
  ...
  const rows = dimensionStats.value.map((dim) => [
    dim.name,
    dim.avg,
    dim.count,
  ])
...
<template>
  <div>
    ...
    <div v-else>
          ...
          <UCard class="p-4">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-2xl font-bold text-[var(--color-text-primary)] mt-1">{{ scoreStats.totalMatches }}</p>
              </div>
          ...
          <UCard class="p-4">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-2xl font-bold text-[var(--color-accent-success)] mt-1">{{ scoreStats.scoredMatches }}</p>
              </div>
          ...
          <UCard class="p-4">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-2xl font-bold text-[var(--color-accent-warning)] mt-1">{{ scoreStats.unscoredMatches }}</p>
              </div>
          ...
          <UCard class="p-4">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-2xl font-bold text-[var(--color-accent-primary)] mt-1">{{ scoreStats.totalJudges }}</p>
              </div>
        ...
        <UCard class="mb-6" v-if="scoreStats">
          ...
          <div class="space-y-2">
            <div class="flex justify-between text-sm">
              <span class="text-[var(--color-text-secondary)]">已完成 {{ scoreStats.scoredMatches }} / {{ scoreStats.totalMatches }} 场评分</span>
        ...
        <UCard class="mb-6" v-if="scoreStats">
          ...
          <div class="grid grid-cols-2 gap-6">
            <div class="text-center p-4 rounded-xl bg-[var(--color-bg-secondary)]">
              ...
              <p class="text-3xl font-bold text-[var(--color-accent-primary)]">{{ scoreStats.averageScoreTeamA }}</p>
            </div>
            <div class="text-center p-4 rounded-xl bg-[var(--color-bg-secondary)]">
              <div class="flex items-center justify-center gap-2 mb-2">
              ...
              <p class="text-3xl font-bold text-[var(--color-accent-error)]">{{ scoreStats.averageScoreTeamB }}</p>
            </div>
          </div>
        </UCard>
          ...
          <div class="space-y-4">
            ...
            >
              <div class="flex justify-between text-sm">
                ...
                <span class="text-[var(--color-text-muted)]">{{ dim.avg }} 分（{{ dim.count }} 次评分）</span>
              </div>
              <div class="h-3 bg-[var(--color-bg-tertiary)] rounded-full overflow-hidden">
                <div
                  class="h-full bg-gradient-to-r from-[var(--color-accent-primary)] to-[var(--color-accent-secondary)] rounded-full transition-all duration-500"
            ...
            <table class="w-full">
              <thead>
                <tr class="border-b border-[var(--color-border)]">
                  <th class="text-left py-3 px-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider">评委</th>
                  <th class="text-center py-3 px-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider">评分场次</th>
                  <th class="text-center py-3 px-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider">平均分</th>
                  <th class="text-center py-3 px-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider">最低分</th>
                  <th class="text-center py-3 px-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider">最高分</th>
                  <th class="text-center py-3 px-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider">波动范围</th>
                </tr>
              </thead>
              <tbody>
                <tr
                ...
                >
                  ...
                  <td class="py-3 px-4 text-center">
                    <span class="text-sm font-medium text-[var(--color-text-primary)]">{{ stats.count }}</span>
                  </td>
                  <td class="py-3 px-4 text-center">
                    <span class="text-sm font-bold text-[var(--color-accent-primary)]">{{ stats.avg }}</span>
                  </td>
                  <td class="py-3 px-4 text-center">
                    <span class="text-sm font-medium text-[var(--color-accent-success)]">{{ stats.min }}</span>
                  </td>
                  <td class="py-3 px-4 text-center">
                    <span class="text-sm font-medium text-[var(--color-accent-error)]">{{ stats.max }}</span>
                  </td>
                  <td class="py-3 px-4 text-center">
                    <span class="text-sm text-[var(--color-text-muted)]">{{ stats.max - stats.min }}</span>
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            >
              <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-3">
                  <span class="text-xs px-2 py-1 rounded-full bg-[var(--color-accent-bg)] text-[var(--color-accent-primary)] font-medium">
                ...
                <span :class="[
                  'text-sm font-medium',
                  item.match.winner === 'teamA' ? 'text-[var(--color-accent-primary)]' :
                  item.match.winner === 'teamB' ? 'text-[var(--color-accent-error)]' :
                  'text-[var(--color-accent-warning)]',
                ]">
                  {{ item.match.winner === 'teamA' ? item.match.teamA : item.match.winner === 'teamB' ? item.match.teamB : '平局' }}
                </span>
              </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  <div class="flex items-center justify-between mb-2">
                    <span class="text-sm font-medium text-[var(--color-text-primary)]">{{ score.judgeName }}</span>
                  ...
                  <div v-if="score.bestDebaterA || score.bestDebaterB" class="mt-2 flex flex-wrap gap-2">
                    <span v-if="score.bestDebaterA" class="text-xs px-2 py-0.5 rounded bg-[var(--color-success-bg)] text-[var(--color-accent-success)]">
                      最佳辩手(正): {{ score.bestDebaterA }}
                    </span>
                    <span v-if="score.bestDebaterB" class="text-xs px-2 py-0.5 rounded bg-[var(--color-error-bg)] text-[var(--color-accent-error)]">
        ...
        <!-- ═══ 无数据提示 ═══ -->
        <div v-if="!loading && (!scoreStats || scoreStats.totalMatches === 0)" class="text-center py-12">
        ...
        <!-- 有数据但无评分详情 -->
        <div v-else-if="!loading && scoreStats?.totalMatches > 0 && !matchScores.length" class="text-center py-12">
          <UIcon name="i-lucide-clipboard-list" class="w-16 h-16 text-[var(--color-text-muted)] mx-auto mb-4" />
