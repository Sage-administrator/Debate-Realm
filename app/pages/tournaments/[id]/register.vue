...
const canIndividual = computed(() => allowedRegType.value === 'individual' || allowedRegType.value === 'both')
...
onMounted(async () => {
  try {
    ...
    toast.add({ title: e?.data?.message || e?.data?.statusMessage || '加载报名信息失败', color: 'error' })
  } finally {
    loading.value = false
  }
})
...
async function handleSubmit() {
  ...
  try {
    ...
    // 收集自定义字段数据（排除系统字段）
    const systemKeys = ['submitterName', 'contactPhone', 'contactEmail', 'teamName', 'notes', 'members']
  ...
  <div class="min-h-screen">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-else-if="!config.registrationOpen" class="glass-card p-8 mt-10 text-center">
        ...
        <p class="text-sm text-[var(--color-text-muted)]">该赛事当前未开放报名，请联系赛事组织者。</p>
      </div>
      ...
      >
        ...
        <p class="text-sm text-[var(--color-text-muted)]">报名截止时间为 {{ formatDate(config.registrationDeadline) }}，已超出截止时间。</p>
      </div>
      ...
      <div v-else-if="submitted" class="glass-card-strong p-10 mt-10 text-center">
        ...
        <div class="bg-[var(--color-bg-secondary)] rounded-lg p-4 mb-6 text-left max-w-sm mx-auto">
          ...
          <div class="flex items-center justify-between text-xs mb-2">
            ...
            <span class="text-[var(--color-text-primary)]">{{ regType === 'team' ? '队伍报名' : '个人报名' }}</span>
          </div>
      ...
      <template v-else>
        ...
        <header class="pt-6 pb-5">
          ...
          <div v-if="config.registrationInfo" class="mt-4 glass-card p-4">
            ...
            <p class="text-sm text-[var(--color-text-primary)] whitespace-pre-line">{{ config.registrationInfo }}</p>
          </div>
        </header>
        ...
        <main class="space-y-6 pb-12">
          ...
          <UCard v-if="canIndividual && canTeam">
            <template #header>
              <h2 class="text-base font-semibold text-[var(--color-text-primary)] flex items-center gap-2">
                <UIcon name="i-lucide-clipboard-list" class="w-4 h-4 text-[var(--color-text-muted)]" />
                选择报名类型
              </h2>
            </template>
          ...
          <template v-for="(field, idx) in visibleFields" :key="field.id || idx">
            ...
            <!-- 装饰元素：分割线 -->
            <div v-if="field.fieldType === 'divider'" class="border-t border-[var(--color-border)] my-2"></div>

            <!-- 装饰元素：分组标题 -->
            <h3 v-else-if="field.fieldType === 'heading'" class="text-base font-bold text-[var(--color-text-primary)] mt-4 mb-1">
              {{ field.fieldName }}
            </h3>
            ...
            <UCard v-else-if="field.fieldType === 'members'">
              <template #header>
                <div class="flex items-center justify-between">
                  <h2 class="text-base font-semibold text-[var(--color-text-primary)] flex items-center gap-2">
                    <UIcon name="i-lucide-users-round" class="w-4 h-4 text-[var(--color-text-muted)]" />
                    {{ regType === 'team' ? '队伍成员' : (field.fieldName || '参赛者信息') }}
                    <span v-if="regType === 'team'" class="text-xs font-normal text-[var(--color-text-muted)]">({{ members.length }} 人)</span>
              ...
              <div class="space-y-4">
                <div v-for="(m, mIdx) in members" :key="mIdx" class="bg-[var(--color-bg-secondary)] rounded-lg p-4">
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div>
                      ...
                      <UInput v-model="m.name" placeholder="成员姓名" class="w-full" :ui="{ base: 'input-glass' }" />
                    </div>
                    <div>
                      <label class="block text-xs text-[var(--color-text-muted)] mb-1">意向职位</label>
                      <!-- USelect 为客户端组件，SSR 渲染会与水合结果不一致 -->
                      <ClientOnly>
                        <USelect v-model="m.preferredPosition" :items="positionOptions" class="w-full" :ui="{ base: 'input-glass' }" />
                        <template #fallback>
                    ...
                    <div>
                      <label class="block text-xs text-[var(--color-text-muted)] mb-1">辩论经历（选填）</label>
                      <UInput v-model="m.experience" placeholder="如：3 年" class="w-full" :ui="{ base: 'input-glass' }" />
                    </div>
                  </div>
                </div>
              </div>
                ...
                <h2 class="text-base font-semibold text-[var(--color-text-primary)] flex items-center gap-2">
                  <UIcon
                    :name="({
                      ...
                      email: 'i-lucide-mail',
                    } as Record<string, string>)[field.fieldType] || 'i-lucide-square'"
                    class="w-4 h-4 text-[var(--color-text-muted)]"
                  />
                  {{ field.fieldName }}
                  <span v-if="field.required" class="text-red-500 text-sm">*</span>
                  <span v-else-if="!['heading', 'divider'].includes(field.fieldType)" class="text-xs font-normal text-[var(--color-text-muted)]">（选填）</span>
                </h2>
              </template>

              <!-- 字段描述 -->
              <p v-if="field.description" class="text-xs text-[var(--color-text-muted)] mb-2">{{ field.description }}</p>
