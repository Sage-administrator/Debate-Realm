// 环节类型统一判断模块 —— 前后端共用的唯一真相来源
//
// 历史问题：stage.type 曾在 schema（5 种）/ 前端（13 种）/ Cascader（8 种）/ typeLabel
// 四处各自定义，取值集合互不一致，导致同一语义可由多种字符串表达，判断逻辑散落各处。
// 本模块收敛为单一实现：所有 type 判断必须 import 自此处，禁止在别处另写一套。
//
// - 前端：经 app/utils/stageType.ts 转发，保留 Nuxt 自动导入
// - 后端：经 server/utils/stageType.ts 转发
// - 测试：vitest.config.ts 已配置 #shared 别名
