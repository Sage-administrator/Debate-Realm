// 获取赛事的报名配置（公开接口，无需鉴权）
// 返回：赛事基础信息 + 报名设置 + 统一字段配置（系统字段 + 自定义字段）
import { prisma } from '../../../lib/prisma'
import { ensureSystemFieldsForTournament } from '../../../utils/system-fields'
...
export default defineEventHandler(async (event) => {
  ...
  // 4. 确保赛事有系统字段记录（首次访问时自动初始化）
  await ensureSystemFieldsForTournament(prisma, id)

  // 5. 查询该赛事的所有报名字段（系统字段 + 自定义字段），按 sortOrder 升序排列
  ...
  return {
    ...
    // 统一字段配置（系统字段 + 自定义字段，按 sortOrder 排序）
