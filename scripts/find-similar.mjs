...
const pattern =
  /class\s*=\s*"([^"]*bg-\[var\(--color-bg-secondary\)\][^"]*rounded[^"]*[pms][by -][^"]*)"/gi
...
for (const file of files) {
  ...
  while ((match = pattern.exec(content)) !== null) {
    ...
    if (
      ...
      !/\bhover\b/.test(classes) &&
      !/\bbackdrop\b/.test(classes) &&
      !/\bshadow\b/.test(classes) &&
      !/\bfixed\b/.test(classes) &&
      !/\bw-\d/.test(classes) &&
      !/\bh-\d/.test(classes) &&
      !/\babsolute\b/.test(classes) &&
      !/\bpeer-\b/.test(classes) &&
      !/\bflex\b/.test(classes) &&
      !/\boverflow\b/.test(classes)
    ) {
