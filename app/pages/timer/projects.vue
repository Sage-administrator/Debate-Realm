  ...
  <div class="min-h-screen">
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 fade-in">
      ...
      <div
        ...
        class="border border-[var(--color-border)] border-dashed rounded-lg p-12 text-center"
      >
        <UIcon
          name="i-lucide-timer"
          class="w-12 h-12 mx-auto text-[var(--color-border-muted)] mb-3"
        />
        <p class="text-[var(--color-text-muted)] mb-4">
          暂无计时器项目，开始创建您的第一个辩论赛配置
        </p>
        <UButton
          color="primary"
          @click="
      ...
      <div v-else class="grid gap-3">
        <div v-for="p in projects" :key="p.id" class="glass-card p-6">
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <h3 class="font-bold text-lg text-[var(--color-text-primary)]">{{ p.name }}</h3>
              <p v-if="p.title" class="text-sm text-[var(--color-text-secondary)] mt-1">
                {{ p.title }}
              </p>
              ...
              <div v-if="p.positiveTopic || p.negativeTopic" class="mt-2 text-sm space-y-1">
                <div v-if="p.positiveTopic" class="text-[var(--color-text-secondary)]">
                  <span class="text-green-600 dark:text-green-400 font-medium">正方：</span
                  >{{ p.positiveTopic }}
                </div>
                <div v-if="p.negativeTopic" class="text-[var(--color-text-secondary)]">
                  <span class="text-blue-600 dark:text-blue-400 font-medium">反方：</span
                  >{{ p.negativeTopic }}
                </div>
              </div>
          ...
          <div class="glass-modal p-6 space-y-4">
            ...
            <div class="flex justify-end gap-2 pt-2">
              <UButton
                ...
                >取消</UButton
              >
              <UButton color="primary" @click="createProject"> 创建 </UButton>
            </div>
          </div>
        </template>
      </UModal>
